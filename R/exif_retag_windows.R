...
#' is 18
#' @param exif_tool_path Full path of exiftool.exe location
#' @importFrom magrittr "%>%"
...
exif_retag_windows <-
  ...
  function(time_diff=0,img_path,log_file,proj_name,csv_out,
           leap_secs= seq(10:50),exif_tool_path) {
    ...
    exif_times <- as.data.frame(system(paste0(exif_tool_path," -T -filename -createdate ",
                                                img_path), inter=TRUE))
    ...
    for (i in 1:nrow(minimal.output))
      {
      system(paste0(exif_tool_path," -overwrite_original -F -q -gpslongitude=",
                  minimal.output$lon[i]," -gpslatitude=",minimal.output$lat[i],
                  " -gpslongituderef=",minimal.output$hor[i]," -gpslatituderef=",
                  minimal.output$ver[i]," -gpsaltitude=",minimal.output$alt[i],
                  " -gpsaltituderef=above ",img_path,"/",minimal.output$name[i]),
           inter=TRUE)
